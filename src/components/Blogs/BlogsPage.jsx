...
import { usePdfGenerator } from "../hookPDF";
...
export function BlogPreview({
  ...
  const { downloadPdf } = usePdfGenerator(); // Use the PDF hook
  ...
  const handleDownloadPDF = () => {
    ...
    const fileName = `acm-blog-${blog.title.toLowerCase().replace(/[^a-z0-9]/g, "-")}`;

    // Download the PDF using your hook
    downloadPdf(blog, "blog", fileName);
  };
        ...
        <CardContent className="pt-6 relative">
          ...
          <div className="mt-6 flex justify-end">
            <Button
              ...
              className="bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-black dark:text-white font-bold py-3 px-6 rounded-lg flex items-center gap-2 transition-all"
            >
...
export default function BlogsPage() {
  ...
  const handleDownloadPDF = (blog, e) => {
